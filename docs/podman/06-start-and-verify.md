# 6. Start and Verify, Podman Stream
...
## Validate the kit before starting

From the `compose/` directory:

```bash
sudo podman compose --profile gateway --profile sra config --quiet
```
...
**What happens:** Podman pulls the images on first run, the gateway image is
...
`podman compose` delegates to the standalone Compose v2 binary, so the
lifecycle behavior matches a Docker host.
...
If the list is empty or the containers are reported missing, the command ran
without `sudo` against the user store; see chapter 2. The bastions take up to
a minute to show `Up` because they wait for the gateway health check. If a
bastion stays `Created` or exits, the gateway did not become healthy;
continue to the next verification to see why.
...
gateway is still starting; wait sixty seconds and retry. If it keeps failing,
check the logs:
...
```bash
sudo podman logs akeyless-gateway 2>&1 | grep -i 'cluster'
```
...
Replace `sra.example.internal` with your container host address from
chapter 2:
...
[Chapter 7: Access Control](../common/07-access-control.md) creates the first
end-user role.
