...
Apply with the recreate sequence from chapter 9 of your stream. If sessions
still drop behind a load balancer, compare the keepalive math against the
balancer's idle timeout: the interval times the count must stay below the
balancer timeout.
...
3. Recreate with the chapter 9 sequence from your stream.
...
SSH bastion deployments can set `ALLOWED_BASTION_URLS` as an allowlist. The
portal removes user-provided endpoint URLs that are not on it. Set it in
`gateway.env` when users should reach bastions only through known addresses.
...
3. Recreate with the sequence from chapter 9 of your stream, then serve the
   console and portal over HTTPS on port 8000 and update every `-g` URL your
   users use to `https://`.

## Next step

Chapter 11 is where the streams split: its commands differ per runtime.
Continue with [chapter 11 of the Docker stream](../docker/11-troubleshooting.md)
or [chapter 11 of the Podman stream](../podman/11-troubleshooting.md) when
