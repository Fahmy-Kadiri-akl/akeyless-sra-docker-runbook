...
```mermaid
flowchart TD
    Q["What changed?"] -->|"A value in gateway.env,<br/>sra.env, or cache.env"| R["docker compose --profile gateway<br/>--profile sra up -d"]
    Q -->|"Gateway application version"| RS["docker restart akeyless-gateway"]
    Q -->|"Base image or Compose file"| P["docker compose pull then<br/>docker compose up -d"]
...
certificates issued, and target connection results.
...
The deployment holds no secrets on the Docker host beyond the environment
